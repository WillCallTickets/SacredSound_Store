...
{
    [ToolboxData("<{0}:Editor_Promoter runat=\"Server\" AbbreviatedDisplay=\"false\" DisplayTitle=\"true\" TitleText=\"PROMOTER EDITOR\" SelectText=\"SELECT PROMOTER\" ></{0}:Editor_Promoter>")]
    public partial class Editor_Promoter : BaseControl
    {
        #region Wizard
        ...
        protected void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            if (Utils.Validation.IsInteger(val))
                Atx.CurrentPromoterId = int.Parse(val);
            else
        ...
        private static readonly object SelectedEditPromoterChangedEventKey = new object();

        public delegate void SelectedEditPromoterChangedEventHandler(object sender, WillCallWeb.Admin.AdminEvent.EditorEntityChangedEventArgs e);

        public event SelectedEditPromoterChangedEventHandler SelectedPromoterChanged
        {
            add { Events.AddHandler(SelectedEditPromoterChangedEventKey, value); }
            remove { Events.RemoveHandler(SelectedEditPromoterChangedEventKey, value); }
        }
        public virtual void OnSelectedPromoterChanged(WillCallWeb.Admin.AdminEvent.EditorEntityChangedEventArgs e)
        {
            SelectedEditPromoterChangedEventHandler handler = (SelectedEditPromoterChangedEventHandler)Events[SelectedEditPromoterChangedEventKey];
        ...
        protected void litImage_DataBinding(object sender, EventArgs e)
        {
            if (wizEdit.ActiveStep.ID.ToLower() == "images")
            {
                ...
                if (useJcrop)
                    jcropId = string.Format("id=\"jcropper-prom\" ");
                ...
                case "savecrop":
                    ...
                    if (Entity != null && Entity.PictureUrl != null && Entity.PictureUrl.Trim().Length > 0)
                    {
                            ...
                            && w1.Trim().Length > 0 && h1.Trim().Length > 0 && x1.Trim().Length > 0 && y1.Trim().Length > 0)
                        {
                            ...
                            int viewedWidth = MaxImageDimension; //(int)vw.First;
        ...
        private string _titleText = "PROMOTER EDITOR";
        ...
        private string _selectText = "SELECT PROMOTER";
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            //set up crop control
            //string script = " $('#jcropper-prom').Jcrop({onSelect: showCoords,onChange: showCoords,bgColor: '#333333',bgOpacity: .7, ";
            ...
            regFunc.AppendFormat("function PromoterItemSelected(source, eventArgs) {{ {0}", Environment.NewLine);
            ...
            //    function PromoterItemSelected(source, eventArgs) {

            //        var selectionPanelId = '<%= this.PanelHidden.UniqueID %>';
            //        var hidden = getChildElement(selectionPanelId, "hidSelectedValue");

            //        hidden.value = eventArgs.get_value();

            //        var elemId = '<%= this.btnLoad.ClientID %>';
            //        var elem = $get(elemId);
        ...
        protected void SqlDetails_Inserted(object sender, SqlDataSourceStatusEventArgs e)
        {
            if (e.Command.Parameters["@NewId"] != null && Utils.Validation.IsInteger(e.Command.Parameters["@NewId"].Value.ToString()))
            {
                ...
                OnSelectedPromoterChanged(new AdminEvent.EditorEntityChangedEventArgs(SelectedIdx, ""));
            }
            else
                SelectedIdx = 0;
        }
        protected void FormView1_ItemInserted(object sender, FormViewInsertedEventArgs e)
        {
            ...
            if(e.Exception != null && e.Exception.Message.ToLower().IndexOf("cannot insert duplicate key row in object") != -1)
            {
                ...
                promo.LoadAndCloseReader(Promoter.FetchByParameter("NameRoot", e.Values["Name"].ToString()));

                //if found
                if (promo.Id > 0)
                {
                    SelectedIdx = promo.Id;
                    e.ExceptionHandled = true;
                    e.KeepInInsertMode = false;
                    FormView view = (FormView)sender;
                    view.ChangeMode(FormViewMode.Edit);

                    OnSelectedPromoterChanged(new AdminEvent.EditorEntityChangedEventArgs(promo.Id, ""));
        ...
        protected void rdoSearch_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            txtSelection.Text = string.Empty;
            Ctx.SearchLike_Promoter = (rbl.SelectedValue == "true");
        }
        ...
        protected void rdoSearch_Load(object sender, EventArgs e)
        {
            ...
            ListItem li = rbl.Items.FindByValue(Ctx.SearchLike_Promoter.ToString().ToLower());
            if (li != null)
                li.Selected = true;
        }
        ...
        protected void btnUpload_Click(object sender, EventArgs e)
        {
            ...
            if (upload != null && upload.HasFile)
            {
                ...
                {   
                    ...
                    string pathFile = string.Format("{0}{1}", _ImageManager._PromoterImageStorage_Local, fileName);
                    ...
                    if (System.IO.File.Exists(mappedFile))
                    {
                        ...
                        mappedFile = Server.MapPath(string.Format("{0}{1}", _ImageManager._PromoterImageStorage_Local, fileName));
                    }
