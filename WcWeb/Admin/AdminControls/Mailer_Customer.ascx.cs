...
{
    public partial class Mailer_Customer : BaseControl
    {
        protected readonly string MailerTypeTitle = "Customer";
        protected readonly string MailTemplateSubDirectory = "CustomerServiceSent";//"AccessCampaignMail";
        protected readonly string[] ParamNames = { "FIRSTNAME", "LASTNAME", "EMAILADDRESS", "INVOICEID" };
        protected readonly string StarterTemplate = _Config._CustomerInquiryTemplate;


        protected List<string> _errors = new List<string>();
        public string _uniqueSessionLetterId = null;
        public string UniqueSessionLetterId 
        { 
            get 
            {
                if (_uniqueSessionLetterId == null)
                    _uniqueSessionLetterId = string.Format("{0}_ControlLetter", this.ClientID);
        ...
        protected void btnLoadTemplate_Click(object sender, EventArgs e)
        {
            if (ddlTemplates.SelectedIndex > 0 && ddlTemplates.SelectedValue.Trim().Length > 0)//allow for list explanation
            {
                ...
                try
                {
                    string mappedPath = Server.MapPath(string.Format("/{0}/MailTemplates/CustomerServiceSent/{1}",
                        _Config._VirtualResourceDir, filename));
                    ...
                    string sql = "SELECT * FROM [EmailLetter] WHERE [ApplicationId] = @appId AND [Name] = @name ";
                    SubSonic.QueryCommand cmd = new SubSonic.QueryCommand(sql, SubSonic.DataService.Provider.Name);
                    cmd.Parameters.Add("@appId", _Config.APPLICATION_ID.ToString(), DbType.String);
                    cmd.Parameters.Add("@name", filename, DbType.String);
        ...
        {
            return GetEmailInputs(insertSample, null, null, null, null);
        }
        private System.Collections.Specialized.ListDictionary GetEmailInputs(bool insertSample, string email)
        {
            return GetEmailInputs(insertSample, null, null, email, null);
        }
        private System.Collections.Specialized.ListDictionary GetEmailInputs(bool insertSample, string firstName, string lastName, string email, string invoiceId)
        {
            ...
            //dict.Add("<% InvoiceId %>", invoice.InvoiceDate.ToString("ddd MM/dd/yyyy hh:mmtt"));
            ...
            dict.Add("<PARAM>FIRSTNAME</PARAM>", (firstName != null && firstName.Trim().Length > 0) ? firstName : (insertSample) ? "sample_FirstName" : string.Empty);
            dict.Add("<PARAM>LASTNAME</PARAM>", (lastName != null && lastName.Trim().Length > 0) ? lastName : (insertSample) ? "sample_LastName" : string.Empty);
            dict.Add("<PARAM>EMAILADDRESS</PARAM>", (email != null && email.Trim().Length > 0) ? email : (insertSample) ? "sample@emailaddress.com" : string.Empty);
            dict.Add("<PARAM>BODY</PARAM>", body);
            dict.Add("<PARAM>CLOSING</PARAM>", (closing.Trim().Length > 0) ? closing.Trim() : "closing goes here");

            dict.Add("<PARAM>INVOICEID</PARAM>", (invoiceId != null && invoiceId.Trim().Length > 0) ? invoiceId : (insertSample) ? "sample_InvoiceId" : string.Empty);
        ...
        {
            return CreateEmailDisplayFromPageValues(null, null, null, null);
        }
        private string CreateEmailDisplayFromPageValues(string email)
        {
            return CreateEmailDisplayFromPageValues(null, null, email, null);
        }
        private string CreateEmailDisplayFromPageValues(string firstName, string lastName, string email, string invoiceId)
        {
            //get template
            ...
            if (file.Trim().Length > 0 && file.IndexOf("Could not find file") == -1)
            {
                ...
                //display
                return Utils.ParseHelper.DoReplacements(file, GetEmailInputs(true, firstName, lastName, email, invoiceId), true);
            }
            else
                return string.Empty;
        }
