...
{
    public partial class ShowTicket_Bundle : BaseControl
    {
        ...
        protected ShowTicket ShowTicketRecord
        {
            get
            {
                if (_showTicketRecord == null)
                {
                    ...
                    string req = this.Request["tixid"];
                    if (req != null && Utils.Validation.IsInteger(req))
                    {
                        List<ShowDate> showDates = new List<ShowDate>();

                        if (Atx.CurrentShowRecord == null)
                        {
                            ShowTicket st = new ShowTicket(int.Parse(req));
                            if (st != null)
                                Atx.SetCurrentShowRecord(st.ShowDateRecord.TShowId);

                            if (Atx.CurrentShowRecord == null)
                                throw new ArgumentException();
                        }

                        showDates.AddRange(Atx.CurrentShowRecord.ShowDateRecords());

                        foreach (ShowDate sd in showDates)
                        {
                            foreach (ShowTicket st in sd.ShowTicketRecords())
                            {
                                if (st.Id.ToString() == req)
                                {
                                    _showTicketRecord = st;
                                    break;
                                }
                            }
        ...
        {
            ((Literal)sender).Text = string.Format("<a href=\"/Admin/ShowEditor.aspx?p=tickets\">Back To Ticket Page</a> <div style=\"font-size:10px;\">Current Ticket: {0}</div>", 
                Utils.ParseHelper.StripHtmlTags(ShowTicketRecord.DisplayNameWithAttribsAndDescription));
        }
        ...
        protected MerchBundle GetCurrentBundle()
        {
            return (GridView1.SelectedValue != null) ?
                (MerchBundle)ShowTicketRecord.MerchBundleRecords().Find((int)GridView1.SelectedValue) : null;
        }
        ...
        protected void GridView1_DataBinding(object sender, EventArgs e)
        {
            ...
            coll.AddRange(ShowTicketRecord.MerchBundleRecords());
            if (coll.Count > 1)
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            switch (cmd)
            {
                case "up":
                case "down":
                    MerchBundle moved = ShowTicketRecord.MerchBundleRecords().ReorderItem(int.Parse(e.CommandArgument.ToString()), cmd);
        ...
        protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            try
            {
                ...
                ShowTicketRecord.MerchBundleRecords().DeleteFromCollection(idx);
                grid.SelectedIndex = (ShowTicketRecord.MerchBundleRecords().Count > 0) ? 0 : -1;
                grid.DataBind();
            }
            catch (Exception ex)
            {
        ...
        protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            MerchBundle entity = (MerchBundle)ShowTicketRecord.MerchBundleRecords().Find(idx);
            ...
            if (entity != null)
            {
                ...
                try
                {
                    ...
                    Atx.RefreshCurrentShowRecord();
                    _showTicketRecord = null;
        ...
        protected void FormView1_DataBinding(object sender, EventArgs e)
        {
            ...
            {
                MerchBundleCollection coll = new MerchBundleCollection();
                MerchBundle pkg = (MerchBundle)ShowTicketRecord.MerchBundleRecords().Find(idx);
                if (pkg != null)
                    coll.Add(pkg);
        ...
        protected void FormView1_ItemInserting(object sender, FormViewInsertEventArgs e)
        {
            ...
            try
            {
                MerchBundle entity = ShowTicketRecord.MerchBundleRecords().AddToCollection(ShowTicketRecord, parentQty, title, comment, dPrice, chkPriced.Checked, maxSelection);
                ...
                GridView1.SelectedIndex = ShowTicketRecord.MerchBundleRecords().Count - 1;                
                GridView1.DataBind();
                updatePanel.Update();
        ...
        protected void ListView1_DataBinding(object sender, EventArgs e)
        {
            ...
            if (GridView1.SelectedDataKey != null)
            {
                ...
                if (idx != -1)
                {
                    MerchBundle bundle = (MerchBundle)ShowTicketRecord.MerchBundleRecords().Find(idx);
        ...
        protected void ListView1_ItemInserting(object sender, ListViewInsertEventArgs e)
        {
            ...
            MerchBundle bundle = (MerchBundle)ShowTicketRecord.MerchBundleRecords().Find(bundleId);
        ...
        protected void ListView1_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            ...
            switch (cmd)
            {
                case "up":
                case "down":
                    MerchBundle bundle = (MerchBundle)ShowTicketRecord.MerchBundleRecords().Find((int)GridView1.SelectedValue);
