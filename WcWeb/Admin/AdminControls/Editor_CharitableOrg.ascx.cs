...
{
    [ToolboxData("<{0}:Editor_CharitableOrg runat=\"Server\" AbbreviatedDisplay=\"false\" DisplayTitle=\"true\" TitleText=\"CHARITABLE ORG EDITOR\" SelectText=\"SELECT CHARITY ORG\" ></{0}:Editor_CharitableOrg>")]
    public partial class Editor_CharitableOrg : BaseControl, IPostBackEventHandler
    {
        ...
        void IPostBackEventHandler.RaisePostBackEvent(string eventArgument)
        {
            ...
            {
                case "rebind":
                    Literal litDesc = (Literal)wizEdit.FindControl("litDesc");
                    litDesc.DataBind();
                    break;
            }
        }
        ...
        protected void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            if (Utils.Validation.IsInteger(val))
                Atx.CurrentCharitableOrgId = int.Parse(val);
            else
                Atx.CurrentCharitableOrgId = 0;

            OnSelectedCharitableOrgChanged(new AdminEvent.EditorEntityChangedEventArgs(Atx.CurrentCharitableOrgId, ""));
            ...
            btnWys.ToolTip = string.Format("/Admin/AdminControls/Wysiwyg/Wysiwyg.aspx?context=co&ctrl={0}", this.UniqueID);
        }
        private static readonly object SelectedEditCharitableOrgChangedEventKey = new object();

        public delegate void SelectedEditCharitableOrgChangedEventHandler(object sender, WillCallWeb.Admin.AdminEvent.EditorEntityChangedEventArgs e);

        public event SelectedEditCharitableOrgChangedEventHandler SelectedCharitableOrgChanged
        {
            add { Events.AddHandler(SelectedEditCharitableOrgChangedEventKey, value); }
            remove { Events.RemoveHandler(SelectedEditCharitableOrgChangedEventKey, value); }
        }
        public virtual void OnSelectedCharitableOrgChanged(WillCallWeb.Admin.AdminEvent.EditorEntityChangedEventArgs e)
        {
            SelectedEditCharitableOrgChangedEventHandler handler = (SelectedEditCharitableOrgChangedEventHandler)Events[SelectedEditCharitableOrgChangedEventKey];
        ...
        protected void litImage_DataBinding(object sender, EventArgs e)
        {
            if (wizEdit.ActiveStep.ID.ToLower() == "images")
            {
                ...
                if (useJcrop)
                    jcropId = string.Format("id=\"jcropper-org\" ");

                int width = MaxImageDimension;
                lit.Text = string.Format("<img src=\"/Images/spacer.gif\" alt=\"\" width=\"{0}px\" height=\"1px\" />", width.ToString());
                ...
                case "savecrop":
                    ...
                    if (Entity != null && Entity.PictureUrl != null && Entity.PictureUrl.Trim().Length > 0)
                    {
                            ...
                            && w1.Trim().Length > 0 && h1.Trim().Length > 0 && x1.Trim().Length > 0 && y1.Trim().Length > 0)
                        {
                            ...
                            int viewedWidth = MaxImageDimension;
                            ...
                            using (MemoryStream ms = new MemoryStream(CropImage, 0, CropImage.Length))
                            {
                                ...
                                {
                                    //save the image to the charity directory
        ...
        //save the id and name of the current charity into object state
        public int SelectedIdx
        {
            get
            {
                return Atx.CurrentCharitableOrgId;
            }
            set
            {
                Atx.CurrentCharitableOrgId = value;
            }
        }
        protected CharitableOrg _org = null;
        protected CharitableOrg Entity
        {
            get
            {
                if (_org == null && SelectedIdx > 0 || (_org != null && _org.Id != SelectedIdx))
                {
                    if (SelectedIdx == 0)
                        _org = null;
                    else
                    {
                        _org = CharitableOrg.FetchByID(SelectedIdx);

                        if (_org != null && _org.ApplicationId != _Config.APPLICATION_ID)
                        {
                            SelectedIdx = 0;
                            _org = null;
                        }
        ...
        private string _titleText = "CHARITABLE ORG EDITOR";
        ...
        private string _selectText = "SELECT CHARITY ORG";
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            //set up crop control
            //string script = " $('#jcropper-org').Jcrop({onSelect: showCoords,onChange: showCoords,bgColor: '#333333',bgOpacity: .7, ";
            ...
            regFunc.AppendFormat("function CharitableItemSelected(source, eventArgs) {{ {0}", Environment.NewLine);
        ...
        //data is bound by a sqldataobject - the key is the admin cookie for currentCharitableId 
        ...
        protected void FormView1_ItemInserted(object sender, FormViewInsertedEventArgs e)
        {
            ...
            //and that is when we are trying to add an existing charity - in this case we would like
            //to just select the charity that was being inserted
            if(e.Exception != null && e.Exception.Message.ToLower().IndexOf("cannot insert duplicate key row in object") != -1)
            {
                //get the charity
                //assign to the editor
                CharitableOrg org = new CharitableOrg();
                org.LoadAndCloseReader(CharitableOrg.FetchByParameter("NameRoot", e.Values["Name"].ToString()));
        ...
        protected void rdoSearch_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            txtSelection.Text = string.Empty;
            Ctx.SearchLike_CharitableOrg = (rbl.SelectedValue == "true");
        }
        ...
        protected void rdoSearch_Load(object sender, EventArgs e)
        {
            ...
            ListItem li = rbl.Items.FindByValue(Ctx.SearchLike_CharitableOrg.ToString().ToLower());
            if (li != null)
                li.Selected = true;
        }
        ...
        protected void btnUpload_Click(object sender, EventArgs e)
        {
            ...
            if (upload != null && upload.HasFile)
            {
                ...
                {   
                    ...
                    string pathFile = string.Format("{0}{1}", _ImageManager._CharityImageStorage_Local, fileName);
                    ...
                    if (System.IO.File.Exists(mappedFile))
                    {
                        ...
                        mappedFile = Server.MapPath(string.Format("{0}{1}", _ImageManager._CharityImageStorage_Local, fileName));
                    }
        ...
        protected void btnUpdateText_Click(object sender, EventArgs e)
        {
            ...
            string newShort = txtShort.Text.Trim();
            if (newShort != Entity.ShortDescription)
            {
                Entity.ShortDescription = newShort;
                isChanged = true;
            }

            string newDescription = litDesc.Text.Trim();
            ...
            int step = wizEdit.ActiveStepIndex;
            wizEdit.DataBind();
            wizEdit.ActiveStepIndex = step;
        }
        protected void txtShort_DataBinding(object sender, EventArgs e)
        {
            TextBox txt = (TextBox)sender;
            txt.Text = (Entity != null) ? Entity.ShortDescription : string.Empty;
        }
        protected void litDesc_DataBinding(object sender, EventArgs e)
        {
            ...
            {
                Literal lit = (Literal)sender;
                litDesc.Text = string.Format("<div class=\"desc-control\">{0}</div>", Entity.Description);
            }
        }
