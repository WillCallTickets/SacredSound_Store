...
{
    public partial class TermsOfSale : System.Web.UI.UserControl
    {   
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                try
                {
                    string mappedPath = Server.MapPath(string.Format("/{0}/Html/TermsOfSale.html", _Config._VirtualResourceDir));
                    doc.Load(mappedPath);
                }
                catch (Exception ex)
                {
